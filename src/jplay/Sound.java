...
public class Sound
{
    ...
    private float volume,volumeinit;
    private boolean volumeChanged,volumeMute,volumeResume;
    private Song song;
                ...
                try
                {
                        ...
                        while( count != -1 && stop == false)
                        {
                                ...
                                if(count > 0 )
                                {
                                    ...
                                    if (volumeMute&&vCheck==1)
                                    {
                                           volumeMute =false;
                                            vCheck=0;
                                            FloatControl volControl = (FloatControl) sourceDataLine.getControl(FloatControl.Type.MASTER_GAIN);
                                            volumeinit = volControl.getValue();
                                            volControl.setValue(volControl.getMinimum());
                                            volumeChanged = false;
                                    }
                                    if (volumeResume&&vCheck==0)
                                    {
                                            volumeResume =false;
                                            vCheck=1;
                                            FloatControl volControl = (FloatControl) sourceDataLine.getControl(FloatControl.Type.MASTER_GAIN);
                                            volControl.setValue(0.0f);
                                            volumeChanged = false;
                                    }
